...
const SKILLS = [
  ...
  { name: "JWT Auth", icon: <FaCode className="w-5 h-5 text-yellow-400" /> },
...
export default function CircularMarqueeCloseRings() {
  ...
  return (
    <section
      id="skills"
      ref={containerRef}
      className="py-10 mt-10 bg-[#050505] relative overflow-hidden sm:min-h-[800px] flex flex-col items-center justify-center"
    >
      {/* CYBER EARTH */}
      ...
      <div className="absolute top-10 text-center z-20">
        ...
        <h2 className="text-4xl md:text-8xl font-bold text-white tracking-tighter uppercase">
          TECH
      ...
      {/* MOBILE MARQUEE */}
      <div className="md:hidden w-full mt-40 mb-20 sm:mb-0 space-y-6 z-10">
      ...
      {/* DESKTOP ORBIT */}
      <div className="hidden md:flex relative z-10 flex-col items-center justify-center w-full">
        ...
        <div className="relative w-full h-[400px] flex items-center justify-center [perspective:1400px]">
          <div className="marquee-ring outer relative w-full h-full flex items-center justify-center preserve-3d">
            {outerSkills.map((skill, i) => {
              ...
              return (
                <div key={i} className="absolute preserve-3d" style={{ transform: `rotateY(${angle}deg) translateZ(${radius}px)` }}>
                  <div className="skill-card flex items-center gap-3 px-5 py-3 bg-black/60 border border-white/10 rounded-xl backdrop-blur-md">
                    <div className="p-2 bg-white/5 rounded-lg">{skill.icon}</div>
                    <span className="text-sm font-mono text-white uppercase">
                      {skill.name}
                    </span>
